...
class DiseaseStateHandler:
    ...
    def _update_stages(self, current_stages_of_individuals):
        ...
        relavent_transition_times = np.array(
            [
                self.transition_times[i, current_stages_of_individuals[i]]
                for i in range(self.num_points)
            ]
        )

        # find transitions to apply
        transitions_to_apply = (current_stages_of_individuals != 0) * (
            self.time_counters > relavent_transition_times
        ).astype("int")
    ...
    def __call__(self, current_stages_of_individuals):
        ...
        (
            new_stages_of_individuals,
            indicies_that_transitioned_to_next_stage,
        ) = self._update_stages(current_stages_of_individuals)
