...
class DiseaseStateHandler:
    ...
    def _sample_transition_times(self) -> None:
        ...
        for i, transition_params in enumerate(TRANSITION_TIMES.values()):
            transition_array[:, i + 1] = transition_params["mean"] + transition_params[
                "std"
            ] * np.random.randn(self.num_points)
        return transition_array

    def _update_stages(
        self, current_stages: np.ndarray
    ) -> Tuple[np.ndarray, np.ndarray]:
        relavent_transition_times = self.transition_times[
            self.point_idx, current_stages
        ]
        transitions_to_apply = (self.time_counters > relavent_transition_times).astype(
            int
        )
        new_stages_of_individuals = np.minimum(current_stages + transitions_to_apply, 5)
    ...
    def __call__(self, current_stages: np.ndarray) -> np.ndarray:
        new_stages_of_individuals = self._update_stages(current_stages)
